...
def _scaled_cn(raw: str, multiplier: str) -> str:
    try:
        value = Decimal(raw.replace(",", "")) * Decimal(multiplier)
    except InvalidOperation:
        return raw
    return number_to_cn(format(value, "f"))


def _signed_word(sign: str) -> str:
    return "上升" if sign in ("+", "＋") else "下降"


def _normalize_contextual_changes(text: str) -> str:
    text = re.sub(
        r"(上涨|上升|增长|增加|升高|走高|反弹)\s*[+＋]?\s*(\d[\d,]*(?:\.\d+)?)\s*%",
        lambda m: f"{m.group(1)}百分之{number_to_cn(m.group(2))}", text,
    )
    text = re.sub(
        r"(下跌|下降|减少|降低|走低|收缩|回落)\s*[-−]?\s*(\d[\d,]*(?:\.\d+)?)\s*%",
        lambda m: f"{m.group(1)}百分之{number_to_cn(m.group(2))}", text,
    )
    text = re.sub(
        r"(上涨|上升|增长|增加|升高|走高|反弹)\s*[+＋]?\s*(\d[\d,]*(?:\.\d+)?)\s*(?:bp|bps|个基点)",
        lambda m: f"{m.group(1)}{number_to_cn(m.group(2))}个基点", text, flags=re.I,
    )
    text = re.sub(
        r"(下跌|下降|减少|降低|走低|收缩|回落)\s*[-−]?\s*(\d[\d,]*(?:\.\d+)?)\s*(?:bp|bps|个基点)",
        lambda m: f"{m.group(1)}{number_to_cn(m.group(2))}个基点", text, flags=re.I,
    )
    return text
...
def _normalize_money(text: str) -> str:
    ...
    text = re.sub(
        r"\$\s*(\d[\d,]*(?:\.\d+)?)\s*万亿",
        lambda m: f"{number_to_cn(m.group(1))}万亿美元", text,
    )
    text = re.sub(
        r"\$\s*(\d[\d,]*(?:\.\d+)?)\s*亿",
        lambda m: f"{number_to_cn(m.group(1))}亿美元", text,
    )
    text = re.sub(
        r"\$\s*(\d[\d,]*(?:\.\d+)?)\s*(trillion|tn)\b",
        lambda m: f"{number_to_cn(m.group(1))}万亿美元", text, flags=re.I,
    )
    # 1 billion USD = 10 亿美元; 1 million USD = 100 万美元.
    text = re.sub(
        r"\$\s*(\d[\d,]*(?:\.\d+)?)\s*(billion|bn)\b",
        lambda m: f"{_scaled_cn(m.group(1), '10')}亿美元", text, flags=re.I,
    )
    text = re.sub(
        r"\$\s*(\d[\d,]*(?:\.\d+)?)\s*(million|mn)\b",
        lambda m: f"{_scaled_cn(m.group(1), '100')}万美元", text, flags=re.I,
    )
    text = re.sub(
        r"\$\s*(\d[\d,]*(?:\.\d+)?)",
...
def normalize_for_tts(text: str) -> str:
    ...
    # Fail-soft fallbacks: symbols should never be handed literally to TTS.
