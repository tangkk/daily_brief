...
def upload_file(client, bucket: str, path: Path, date: str):
    ...
    # Verify the object exists before recording it in the manifest.
    head = client.head_object(Bucket=bucket, Key=key)
    size = int(head.get("ContentLength", 0))
    if size <= 0:
        raise RuntimeError(f"Uploaded object has invalid size: {key}")
    print(f"OK {key} ({size} bytes)")
    return public_url(date)


def sync_local(audio_dir: Path, manifest_path: Path):
    client = r2_client()
    ...
    for path in sorted(audio_dir.glob("*-daily-brief.mp3")):
        ...
        url = upload_file(client, bucket, path, date)
...
def main():
    ...
    args = ap.parse_args()
    sync_local(Path(args.audio_dir), Path(args.manifest))
